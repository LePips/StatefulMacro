...
extension StatefulMacro {
    ...
    static func getConformances(from node: AttributeSyntax) -> [String] {
        guard let arguments = node.arguments?.as(LabeledExprListSyntax.self) else {
            return []
        }

        let conformancesArgument = arguments.first {
            argument in
    ...
    static func findActionEnum(in declaration: some DeclGroupSyntax) -> (actionEnum: EnumDeclSyntax?, isCasePathable: Bool) {
        ...
        let isCasePathable = actionEnum.attributes.contains {
            attr in
    ...
    static func findEventEnum(in declaration: some DeclGroupSyntax) -> EnumDeclSyntax? {
        declaration.memberBlock.members.compactMap { $0.decl.as(EnumDeclSyntax.self) }
            .first(where: { $0.name.text == "Event" })
    }

    static func handleBackgroundState(in declaration: some DeclGroupSyntax, access: String) throws -> (String, EnumDeclSyntax?) {
    ...
    static func handleEventEnum(in declaration: some DeclGroupSyntax, access: String) throws -> (String, EnumDeclSyntax?, Bool) {
        ...
        if let userDefinedEventEnum {
            ...
            let newCases = userDefinedEventEnum.memberBlock.members.filter {
                member in
    ...
    static func modifyTransitionVariable(_ transitionVariable: VariableDeclSyntax) -> VariableDeclSyntax {
    ...
    static func generateActionFunctions(
        from stateActionEnums: [EnumDeclSyntax],
        in declaration: some DeclGroupSyntax,
        ...
        let hasCancelAction = allCases.contains {
            caseDecl in
            caseDecl.elements.contains { $0.name.text == "cancel" }
        }

        let hasErrorAction = allCases.contains {
            caseDecl in
        ...
        let nonErrorCases = allCases.filter {
            caseDecl in
            !caseDecl.elements.contains { $0.name.text == "error" } &&
                !caseDecl.elements.contains { $0.name.text == "cancel" }
        }
    ...
    static func buildFunction(_ functionDecl: String, _ stmtSyntax: String) throws -> DeclSyntax {
    ...
    static func processFunctionAttributes(
        in declaration: some DeclGroupSyntax,
    ...
    static func createCoreProperty(
        stateEnumName: String,
    ...
    static func createPublishedProperties(
        in declaration: some DeclGroupSyntax,
        stateEnumName: String,
    ...
    static func createPublisherAssignments(
        hasError: Bool,
    ...
    static func generateFunctionCall(
        functionName: String,
    ...
    static func createBackgroundStruct(
